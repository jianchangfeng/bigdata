...
public class HBaseToHDFSZuoYe {
    public static void main(String[] args) throws Exception {
        ...
        Path outputPath = new Path("/student/MorF_avgresult");
    ...
    public static class HBaseToHDFSMapper extends TableMapper<Text, IntWritable> {
        ...
        protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
            ...
            if(isContainsColumn) {
                ...
                List<Cell> sexListCells = value.getColumnCells("info".getBytes(), "sex".getBytes());
                Cell sexCell = sexListCells.get(0);
                byte[] cloneValue = CellUtil.cloneValue(cell);
                String ageValue = Bytes.toString(cloneValue);
                byte[] cloneSexValue = CellUtil.cloneValue(sexCell);
                String sexValue = Bytes.toString(cloneSexValue);
                outKey.set(sexValue);
