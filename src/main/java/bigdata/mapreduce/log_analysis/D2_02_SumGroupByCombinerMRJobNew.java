...
public class D2_02_SumGroupByCombinerMRJobNew extends Configured implements Tool {
    ...
    //Combiner优化
    public static class SumGroupByCombiner extends Reducer<Text, D2_AdMetricWritable, Text, D2_AdMetricWritable>{
        @Override
    ...
    public static class SumGroupByReducer extends Reducer<Text, D2_AdMetricWritable, Text, D2_AdMetricWritable>{
        ...
        protected void reduce(Text key, Iterable<D2_AdMetricWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for(D2_AdMetricWritable adMetric : values){
                ...
                System.out.println(key.toString() + " -> " + adMetric.getPv() + "," + adMetric.getClick());
                pv += adMetric.getPv();
                click += adMetric.getClick();
            }
    ...
    public int run(String[] args) throws InterruptedException, IOException, ClassNotFoundException {
        ...
        job.setJarByClass(D2_02_SumGroupByCombinerMRJobNew.class);
    ...
    public static void main(String[] args) {
        ...
        if(args.length == 0){
            args = new String[]{
                    ...
                    "hdfs://ns/mr_project/log_analysis/combiner_output2-2"
            };
        }
        //1.配置job
        ...
        try {
            int status = ToolRunner.run(conf,new D2_02_SumGroupByCombinerMRJobNew(),args);
            System.exit(status);
        } catch (Exception e) {
            e.printStackTrace();
        }
